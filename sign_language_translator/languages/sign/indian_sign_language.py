"""Defines a class for constructing Indian Sign Language from text using rules."""
...
class IndianSignLanguage(SignLanguage):
    """A class representing the Indian Sign Language.

    It provides methods for converting tokens to sign dictionaries and restructuring sentences.

    Attributes:
        STOPWORDS (set): A set of stopwords in Indian Sign Language.
    """

    STOPWORDS = {"the", "so", "is", "am", "are"}

    @staticmethod
    def name() -> str:
        return SignLanguages.INDIAN_SIGN_LANGUAGE.value
    ...
    def _apply_rules(
        ...
        if sign is None:
            raise ValueError(f"No IndianSL sign could be inferred for {token = }.")
    ...
    def restructure_sentence(
        ...
        # map to Indian grammar (SOV ideally)
