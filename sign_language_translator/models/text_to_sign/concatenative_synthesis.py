...
class ConcatenativeSynthesis(TextToSignModel):
    ...
    def __init__(
        ...
        """
        Args:
            text_language (str | TextLanguage | Enum): (source) The text language processor object or its identifier. (e.g. "english" or `slt.languages.text.English()`. See `slt.TextLanguageCodes` for all options.)
            sign_language (str | SignLanguage | Enum): (target) The sign language processor object or its identifier. (e.g. "in-sl" or `slt.languages.sign.IndianSignLanguage()`. See `slt.SignLanguageCodes` for all options.)
